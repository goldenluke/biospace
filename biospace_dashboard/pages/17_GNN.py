...
st.set_page_config(page_title="GNN · BioSpace", page_icon="🕸️", layout="wide")
...
st.caption(
    ...
    "Prevê fenótipo de forma semi-supervisionada e transdutiva: alguns pacientes têm rótulo "
    "(treino), outros não, mas TODOS participam da propagação de mensagens."
)
...
st.caption(
    "⚠️ Achado real deste projeto: com MUITOS rótulos, o grafo pode ATRAPALHAR (suaviza fronteiras "
    "já bem definidas nas Features); com POUCOS rótulos, o grafo AJUDA bastante. Experimente frações baixas (~5%) para ver a vantagem."
)
...
if "_gnn_resultado" in st.session_state:
    ...
    if diferenca > 0.02:
        st.success(f"O grafo AJUDOU (+{100*diferenca:.1f} pontos percentuais) — esperado com poucos rótulos.")
    elif diferenca < -0.02:
        st.info(f"O grafo ATRAPALHOU levemente ({100*diferenca:.1f} pontos percentuais) — esperado com MUITOS rótulos (fronteiras já bem definidas nas Features).")
    else:
