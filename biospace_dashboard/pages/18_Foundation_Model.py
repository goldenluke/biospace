...
st.set_page_config(page_title="Foundation Model · BioSpace", page_icon="🧩", layout="wide")
...
st.error(
    ...
    "diferença é de ORDEM DE GRANDEZA, não de grau. O valor aqui é mostrar que a representação "
    "(Features com proveniência semântica) é um substrato válido para pré-treino auto-supervisionado."
)
st.caption(
    "Masked Feature Prediction — o mesmo padrão do BERT (Devlin et al., 2019), mas mascarando "
    "Features fisiológicas em vez de palavras. Mascara uma fração aleatória das Features de cada "
    "paciente e treina uma rede para reconstruí-las a partir do resto do perfil."
)
...
if st.button("Treinar Masked Feature Predictor", type="primary"):
    ...
    with st.spinner("Treinando (várias máscaras aleatórias por paciente)..."):
...
if "_masked_resultado" in st.session_state:
    ...
    st.metric("MSE global (todas as Features mascaradas)", f"{resultado['mse_global']:.3f}")

    st.markdown("**Melhor reconstruídas** (razão baixa = boa — o resto do perfil já revela essa Feature):")
    st.dataframe(df_resultado.head(10), use_container_width=True, hide_index=True)

    st.markdown("**Pior reconstruídas** (razão perto de 1 = a Feature carrega informação pouco redundante):")
    st.dataframe(df_resultado.tail(10), use_container_width=True, hide_index=True)

    fig = px.bar(df_resultado, x="feature", y="razao", title="Razão MSE/variância por Feature (menor = melhor reconstruída)")
    fig.add_hline(y=1.0, line_dash="dash", line_color="red", annotation_text="baseline (prever a média)")
    ...
    features_para_mascarar = st.multiselect("Features para mascarar (esconder e tentar prever)", modelo.feature_names_, default=modelo.feature_names_[:3])
